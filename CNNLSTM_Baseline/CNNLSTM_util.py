...
from CNN_util import split_sequences,get_data,seed_everything
...
class Net(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_size, num_layers, output_size, batch_size, kernel_size,seq_length,dropout=0.2) -> None:
        ...
        self.conv = nn.Conv1d(in_channels=self.in_channels, out_channels=self.out_channels*100, kernel_size=self.kernel_size,device=device )
            #----------------------add by funing ---------------------
            # nn.Conv1d(self.out_channels, self.out_channels, self.kernel_size, stride=3),
            # nn.BatchNorm1d(self.out_channels),
            # nn.Conv1d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=self.kernel_size),
            # # nn.BatchNorm1d(32),
            # nn.MaxPool1d(self.out_channels)
            # ----------------------add by funing ---------------------
            # shape(7,--)  ->(64,3,2)
            # nn.ReLU()

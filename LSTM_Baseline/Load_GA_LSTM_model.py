...
class HybridMlp:
    ...
    def prediction_value(self):
        ...
        file ="2852.2179965774003"#str(str(self.best_fit[0])) #str(self.best_fit[0])
        # file = file.replace('[', '').replace(']', '')
        location = "./model/" + file + "_weight.pth"  #

        n_steps =2 #int(structure["n_steps"])
        ...
        np.savetxt("GA_ytest_pred.csv", y_test_pred, delimiter=',')
        np.savetxt("GA_ytest.csv", y_test, delimiter=',')
